...
int xdp_lb(struct xdp_md *ctx)
{
	...
	if (src_ip == 0xa000001){
		uint32_t hash_value = (src_ip ^ src_port ^ dst_port ^ protocol) % 2;
		if (hash_value==0){
			dst_ip = 0xa000002;
		}else{
			dst_ip = 0xa000003;
	...
	ip->src_addr = 0xa00000a;
	ip->dst_addr = dst_ip;

	ip->check = 0;
	__u32 csum = 0;
	csum = bpf_csum_diff(0,0,(__be32 *)ip, sizeof(struct iphdr), 0);
	ip->check = ~csum_reduce_helper(csum)
